...
import { fetchDeptHeadAnalytics, type DeptHeadAnalytics } from '../api/analytics';
import { fetchDeptHeadComplaints, type Complaint } from '../api/complaints';
...
export default function DeptHeadDashboard() {
  ...
  useEffect(() => {
    fetchDeptHeadAnalytics().then(setAnalytics).catch(console.error);
    fetchDeptHeadComplaints().then(setComplaints).catch(console.error);
  }, []);
  ...
  // Generate chart data based on complaint creation dates
      ...
      <nav className="fixed left-0 top-0 h-screen w-64 flex-col border-r border-outline-variant py-8 bg-surface-container-low hidden md:flex z-40">
        <div className="px-4 mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-body-sm text-body-sm text-on-surface-variant">Dept Head Portal</p>
          </div>
        </div>
      ...
      <div className="flex-1 md:ml-64 flex flex-col min-h-screen bg-surface">
        ...
        <main className="flex-1 p-8 overflow-y-auto">
          {activeTab === 'dashboard' && (
            <>
              <div className="mb-8">
                <h2 className="font-headline-lg text-headline-lg text-primary">Department Dashboard</h2>
                <p className="font-body-md text-body-md text-on-surface-variant">Real-time overview for: <span className="font-bold text-primary">{analytics?.department || 'Loading...'}</span></p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col shadow-sm">
                  <div className="flex justify-between items-start mb-4">
                    <span className="font-label-md text-label-md text-on-surface-variant">Total Complaints</span>
                ...
                <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col shadow-sm">
                  <div className="flex justify-between items-start mb-4">
                    <span className="font-label-md text-label-md text-on-surface-variant">Escalations</span>
                    <span className="material-symbols-outlined text-error">warning</span>
                  </div>
                  <div className="font-display-lg text-display-lg text-primary">{escalations.length}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 flex flex-col gap-6">
                  <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm">
                    <h3 className="font-headline-md text-headline-md text-primary mb-4 border-b border-outline-variant pb-2">Recent Department Issues</h3>
                        ...
                        <tbody className="font-body-sm text-body-sm text-on-surface">
                          ...
                          {complaints.slice(0, 10).map(c => (
                            <tr key={c.id} onClick={() => { setSelectedComp(c); setShowModal(true); }} className="border-b border-outline-variant/50 hover:bg-primary/10 hover:shadow-xs cursor-pointer transition-all duration-150">
                              ...
                              <td className="py-2.5">{new Date(c.createdAt).toLocaleDateString()}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          {activeTab === 'map' && (
            <div className="h-[calc(100vh-120px)] flex flex-col">
              <div className="mb-4">
                <h2 className="font-headline-lg text-headline-lg text-primary">Department Jurisdiction Map</h2>
                <p className="font-body-md text-body-md text-on-surface-variant">Live view of all active issues under your department.</p>
              </div>
          ...
          {activeTab === 'analytics' && (
            <div>
              <div className="mb-8">
                ...
                <p className="font-body-md text-body-md text-on-surface-variant">Historical trends and performance tracking.</p>
              </div>
